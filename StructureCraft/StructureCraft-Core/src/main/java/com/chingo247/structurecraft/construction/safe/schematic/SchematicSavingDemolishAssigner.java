...
import static com.chingo247.structurecraft.construction.safe.schematic.SchematicSavingAssigner.CHUNK_SIZE;
    ...
    public void assignTasks(AsyncEditSession session, UUID playerOrRandomUUID, IConstructionEntry constructionEntry) throws StructureException, IOException {
        ...
        for(CuboidRegion region : chunks) {
            ...
            constructionEntry.addTask(new AWEPlacementTask(
                    asyncWorldEdit, constructionEntry, new SchematicSubPlacement(placement, region), playerOrRandomUUID, session, structure.getMin(), new SafeDemolishCallback(constructionEntry, progress))
            );
