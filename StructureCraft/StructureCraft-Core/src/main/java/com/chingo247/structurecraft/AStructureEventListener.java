 ...
 * @author Chingo
 */
public class AStructureEventListener {
    
    private static final Logger LOG = Logger.getLogger(AStructureEventListener.class.getName());
    
    protected static final DecimalFormat TWO_DECIMAL = new DecimalFormat("#.##");
    protected final IEconomyProvider economyProvider;
    protected final KeyPool<Long> structureEventPool;

    public AStructureEventListener(IEconomyProvider economyProvider) {
    ...
    protected void handleStructure(final IStructure structure, final String message, final ConstructionStatus newStatus) {
            ...
            public void run() {
                try {
                    ...
                    if (oldStatus != newStatus) {
                        StructureAPI.getInstance().getEventDispatcher().dispatchEvent(new StructureStateChangeEvent(structure));
                    }
